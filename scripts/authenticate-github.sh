...
token_url="https://github.com/settings/personal-access-tokens/new?target_name=&name=$(uri_encode "$token_name")&description=$(uri_encode "$token_description")&expires_in=$(uri_encode "$token_expires_in")&contents=write&pull_requests=write&issues=read&actions=read&statuses=read"
